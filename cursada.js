...
const Cursada = (() => {
  ...
  function renderForm() {
    ...
    el.innerHTML = `
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-bottom:12px">
        ...
        <label style="display:flex;align-items:center;gap:5px;font-size:11px;color:var(--tt);cursor:pointer">
  ...
  function filaEdit(it) {
    ...
    return `<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;padding:8px;border:1px solid var(--accent,#3B82F6);border-radius:8px;margin-bottom:6px">
      ...
      <label style="display:flex;align-items:center;gap:5px;font-size:10.5px;color:var(--tt);cursor:pointer">
  ...
  function filaActiva(it) {
    ...
    const rep = it.repeat === 'weekly' ? `<span style="font-size:10px;color:var(--tt)"> · 🔁 semanal${it.hechas ? ` (${it.hechas})` : ''}</span>` : '';
    return `<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:8px;border:1px solid rgba(255,255,255,.08);border-radius:8px;margin-bottom:6px">
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer;flex:1;min-width:0">
        ...
        <span style="font-size:12px">${badge(it)} <b>${esc(it.materia)}</b>${it.titulo ? ` <span style="color:var(--tt)">· ${esc(it.titulo)}</span>` : ''} <span style="font-size:10px;color:var(--tt)">· ${fmtF(it.fecha)}${it.hora ? ' ' + esc(it.hora) : ''}${rep}</span></span>
      </label>
      <span style="white-space:nowrap;font-size:12px">${etiqueta(it)}
  ...
  function renderList() {
    ...
    el.innerHTML = `
      <div style="font-size:9px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--tt);margin-bottom:6px">Pendientes (${act.length})</div>
