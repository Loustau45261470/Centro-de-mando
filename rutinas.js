...
function renderRutinas() {
  ...
  const cards = S.routines.map(r => {
    ...
    const lastDateLabel = last ? `<span style="font-family:var(--mono);font-size:12px;color:var(--tt);flex-shrink:0;margin-right:6px">${last.date.slice(5)}</span>` : '';
      ...
      <div class="rtn-card" id="rtn-card-${r.id}">
        <div class="rtn-header" onclick="toggleRutina('${r.id}')">
          ...
          <div class="rtn-info">
            ...
            <div class="rtn-preview" style="margin-top:1px;font-size:12px;color:var(--tt)">${r.exercises.length} ejercicios</div>
          </div>
          <button class="icon-btn" onclick="event.stopPropagation();openEditRoutine('${r.id}')" style="color:var(--tt);flex-shrink:0">
            <svg viewBox="0 0 24 24" style="width:15px;height:15px"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </button>
          <button class="btn btn-ghost" onclick="event.stopPropagation();openExPicker('add','${r.id}')" style="font-size:11.5px;padding:2px 7px;flex-shrink:0;line-height:1.4">+ Ej</button>
    ...
    <div style="padding:12px 14px 16px">
      ...
      <div style="margin-bottom:14px">
        <div onclick="toggleMuscleDist()" style="display:flex;align-items:center;gap:6px;cursor:pointer;margin-bottom:6px;user-select:none">
          <span id="muscle-dist-chev" style="font-size:11.5px;color:var(--tt);display:inline-block;transition:transform .2s;transform:${_muscleDistOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}">▾</span>
          <span style="font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--tt)">Distribución muscular</span>
        </div>
            ...
            : '<div class="scan-duo">' + buildSpinViewer() + buildMuscleHeatmap(_muscleCounts) + '</div>' + _muscleArr.map(([muscle, count]) => {
                ...
                return `<div style="margin-bottom:6px">
                  <div style="display:flex;justify-content:space-between;margin-bottom:2px">
                    <span style="font-size:12px;color:var(--fg)">${muscle}</span>
                    <span style="font-family:var(--mono);font-size:11.5px;color:var(--tt)">${pct}%</span>
                  </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px">
        <div style="background:var(--bg2);border-radius:10px;padding:10px 12px">
          ...
          <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--tt);margin-top:5px">Entrenamientos</div>
        </div>
        <div style="background:var(--bg2);border-radius:10px;padding:10px 12px">
          ...
          <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--tt);margin-top:5px">Volumen kg</div>
        </div>
        <div style="background:var(--bg2);border-radius:10px;padding:10px 12px">
          <div style="font-family:var(--mono);font-size:22px;font-weight:800;color:var(--c-salud);line-height:1">${_totalSets}</div>
          <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--tt);margin-top:5px">Series</div>
        </div>
        <div style="background:var(--bg2);border-radius:10px;padding:10px 12px">
          <div style="font-family:var(--mono);font-size:22px;font-weight:800;color:var(--c-salud);line-height:1">${_timeStr}</div>
          <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--tt);margin-top:5px">Tiempo</div>
        </div>
      </div>
      <div style="margin-top:16px">
        <div onclick="toggleRecords()" style="display:flex;align-items:center;gap:6px;cursor:pointer;margin-bottom:6px;user-select:none">
          <span id="records-chev" style="font-size:11.5px;color:var(--tt);display:inline-block;transition:transform .2s;transform:${_recordsOpen ? 'rotate(0deg)' : 'rotate(-90deg)'}">▾</span>
          <span style="font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--tt)">Records</span>
        </div>
          ...
          ${_recArr.length === 0
            ...
            : _recArr.map(rec => `
              <div style="padding:9px 0;border-top:1px solid var(--border)">
                <div style="font-size:12px;font-weight:700;color:var(--fg);margin-bottom:6px">${rec.name}</div>
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">
                  <div>
                    <div style="font-family:var(--mono);font-size:14px;font-weight:800;color:var(--c-salud);line-height:1">${rec.maxWeight}<span style="font-size:11px;font-weight:400"> kg</span></div>
                    <div style="font-size:11px;color:var(--tt);margin-top:2px">${rec.maxWeightDate ? rec.maxWeightDate.slice(5) : '—'} · peso</div>
                  </div>
                  <div>
                    <div style="font-family:var(--mono);font-size:14px;font-weight:800;color:var(--c-salud);line-height:1">${rec.maxReps}<span style="font-size:11px;font-weight:400"> reps</span></div>
                    <div style="font-size:11px;color:var(--tt);margin-top:2px">${rec.maxRepsDate ? rec.maxRepsDate.slice(5) : '—'} · reps</div>
                  </div>
                  <div>
                    <div style="font-family:var(--mono);font-size:14px;font-weight:800;color:var(--c-salud);line-height:1">${rec.maxVol >= 1000 ? (rec.maxVol/1000).toFixed(1)+'<span style="font-size:11px;font-weight:400">t</span>' : rec.maxVol+'<span style="font-size:11px;font-weight:400"> kg</span>'}</div>
                    <div style="font-size:11px;color:var(--tt);margin-top:2px">${rec.maxVolDate ? rec.maxVolDate.slice(5) : '—'} · kg×reps</div>
                  </div>
                </div>
              </div>`).join('')}
        </div>
  ...
  wrap.innerHTML = `
    <div class="card">
      ...
      <div class="rtn-section-hdr" onclick="toggleRtnSection('rutinas')">
        ...
        <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openModal('modal-add-routine')" style="font-size:11px;letter-spacing:.06em">+ Nueva</button>
      </div>
      ...
      <div class="rtn-section-hdr" onclick="toggleRtnSection('biblioteca')">
        ...
        <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openLibEx()" style="font-size:11px;letter-spacing:.06em">+ Nuevo</button>
      </div>
...
function buildSpinViewer() {
  return `<div style="margin:6px 0 18px">
    <div style="font-size:10.5px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:rgba(255,150,40,.6);margin-bottom:8px">◈ GIRO 360°</div>
...
function buildMuscleHeatmap(muscleCounts) {
  ...
  return `<div style="margin:10px 0 6px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
      <span style="font-size:10.5px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:rgba(0,200,255,0.45)">◈ MAPA DE ENTRENAMIENTO</span>
      <span style="font-size:10.5px;color:rgba(0,200,255,0.4)">toca un músculo</span>
    </div>
    ...
    <div style="display:flex;align-items:center;gap:8px;justify-content:center;margin-top:9px">
      <span style="font-size:11px;color:var(--ts)">Menos</span>
      ...
      <span style="font-size:11px;color:var(--ts)">Más</span>
    </div>
  </div>`;
}
...
function scanMuscleTap(ev, muscle, viewKey) {
  ...
  const rows = data
    ...
    : `<div style="font-size:11.5px;color:rgba(150,180,200,.7);padding:2px 0">Sin entrenar en este período</div>`;
  const below = y < 30;
...
function openExHistByLib(libId) {
  ...
  const maxVolStr = maxVol >= 1000 ? `${(maxVol / 1000).toFixed(1)}<span style="font-size:12px;font-weight:400"> t</span>` : `${maxVol}<span style="font-size:12px;font-weight:400"> kg</span>`;
  ...
  const chartsHtml = chartData.length >= 2
    ...
    : '<div style="font-size:12px;color:rgba(0,200,255,.35);padding:12px 0;text-align:center">Se necesitan al menos 2 sesiones para los gráficos.</div>';
  ...
  document.getElementById('modal-ex-hist-body').innerHTML = `
    <div style="position:relative;overflow:hidden">
      ...
      <div class="ex-hud-kpi-row">
        ...
        <div class="ex-hud-kpi">
          <div class="ex-hud-kpi-val">${maxWeight}<span style="font-size:12px;font-weight:400"> kg</span></div>
  ...
  if (chartData.length >= 2) {
    ...
    const tick = { color: 'rgba(255,255,255,.5)', font: { size: 11 } };
    ...
    if (wrCanvas) _exHistCharts.push(new Chart(wrCanvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: { legend: { labels: { color: 'rgba(255,255,255,.65)', font: { size: 12 }, boxWidth: 12 } } },
        scales: {
          x: { grid, ticks: tick },
      ...
      <div class="rtn-ex-block">
        ...
        <div class="rtn-ex-block-body" id="rtn-exbody-${ex.id}" style="${isExpanded ? '' : 'display:none'}">
          ...
          <div style="${ex.notes ? 'margin-top:8px' : ''}">
            <div class="rtn-sets-header"><div>Serie</div><div>KG</div><div>Reps</div><div style="text-align:center;font-size:11.5px">✓</div><div></div></div>
...
function renderGymConfigModal() {
  ...
  grid.innerHTML = days.map(d=>`
    <div class="split-day">
      <label>${names[d]}</label>
      <select id="split_${d}" class="inp" style="padding:4px;font-size:11.5px">
...
function drawExerciseChart(exId, dates) {
  ...
  new Chart(ctx.getContext('2d'), {
    ...
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { font: { size: 11 } } }, y: { ticks: { font: { size: 11 }, callback: v => v + ' kg' } } } }
  });
}
