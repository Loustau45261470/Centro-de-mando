...
const SGC = (() => {
  ...
  function barras(tend, std) {
    ...
    }).join('') + `</div><div style="font-size:10.5px;color:var(--tt);letter-spacing:.08em">TENDENCIA 4 SEMANAS · línea de estándar ${std}${std === STD.gym ? '%' : '%'}</div>`;
  }
  ...
  function bloque(titulo, m, std, warn, unidad, alerta, extra, csvKey) {
    ...
    const alertHtml = (m.val != null && m.val < warn) ? `<div style="font-size:11.5px;color:#F43F5E;margin-top:4px">🔔 ${alerta}</div>` : '';
    return `<div style="border-top:1px solid rgba(255,255,255,.06);margin-top:10px;padding-top:8px">
      <div style="display:flex;justify-content:space-between;align-items:baseline">
        <span style="font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--tt)">🛡️ SGC · ${titulo}</span>
        <span style="font-size:10.5px;color:var(--tt)">${semana}</span>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px">
        ...
        <span style="font-size:11px;color:var(--tt)">estándar ${std === STD.gym ? '+' + std + '% mensual' : std + '%'}</span>
      </div>
      ${extra || ''}${alertHtml}
      <div style="text-align:right;margin-top:4px"><button class="btn btn-ghost btn-sm" style="font-size:11px;padding:2px 8px" onclick="SGC.exportCSV('${csvKey}')">⬇ CSV</button></div>
    </div>`;
  }
  ...
  function renderAnalisis(sec) {
    ...
    if (sec === 'finanzas') {
      ...
      el.innerHTML = bloque('Precisión de proyecciones (YTD)', m, STD.inv, WARN.inv, '%',
        ...
        `${barras(tend, STD.inv)}<div style="font-size:11.5px;color:var(--tt);margin-top:3px">${m.n} resueltas · ${pend} pendientes — registro en la <b>Cartera de inversión</b></div>`, 'inv');
    }
    if (sec === 'conocimiento') {
      const m = metricEst();
      const tend = tendencia4Sem((S.sgc.estudio || []).filter(s => s.retencion != null), s => s.fecha, s => s.retencion);
      const deb = m.debiles.length ? `<div style="font-size:11.5px;color:var(--tt);margin-top:3px">Temas débiles: ${m.debiles.slice(0, 3).map(([t, v]) => `<b>${esc(t)}</b> (${v.toFixed(0)}%)`).join(' · ')}</div>` : '';
      el.innerHTML = bloque('Retención de estudio (4 sem.)', m, STD.est, WARN.est, '%',
        ...
        barras(tend, STD.est) + deb + (m.pendientes.length ? `<div style="font-size:11.5px;color:#FBBF24;margin-top:3px">📝 ${m.pendientes.length} repaso(s) pendiente(s) en la card Sesiones de estudio</div>` : ''), 'est');
    }
    if (sec === 'salud') {
      const m = metricGym();
      ...
      el.innerHTML = bloque(`Progresión de volumen (${m.mesAnt} → ${m.mesAct})`, m, STD.gym, WARN.gym, '%',
        ...
        `<div style="font-size:11.5px;color:var(--tt);margin-top:3px">${m.items.length
      ...
      ${ultimas.length ? `<div class="est-sec-lbl">Últimas sesiones</div>` +
        ultimas.map(s => {
          const right = s.retencion == null
            ...
            : `<span class="est-ret ${s.retencion >= STD.est ? 'ok' : 'bad'}">${s.retencion.toFixed(0)}<span style="font-size:11.5px">%</span></span>`;
  ...
  function formHtml() {
    return `
      ...
      <button class="est-btn est-btn-ghost" style="flex:none;font-size:12px;padding:7px 13px;margin-top:2px" onclick="SGC.masPregunta()">+ pregunta</button>
      <div class="est-actions">
  ...
  function quizHtml() {
    ...
    return `<div class="est-quiz">
      ...
      <div class="est-quiz-exit"><button class="est-btn est-btn-ghost" style="flex:none;font-size:11.5px;padding:6px 13px" onclick="SGC.salirQuiz()">Salir (retomar después)</button></div>
    </div>`;
  }
  ...
  function renderProyecciones(el, carteraData) {
    ...
    const filaPend = p => {
      ...
      return `<div class="ci-row" style="grid-template-columns:1fr .9fr 1fr 1.3fr">
        ...
        <span style="text-align:right">${vencida
          ...
          : `<span style="font-size:12px;color:var(--text-ter,#8b93a7)">en ${diasEntre(hoy(), p.fechaVence)} días</span>`}
    ...
    const filaHist = p => { const prec = precisionDe(p); return `<div class="ci-row" style="grid-template-columns:1fr .9fr .9fr .9fr .6fr">
      <span><b>${esc(p.simbolo)}</b> <span style="font-size:11.5px;color:var(--text-ter,#8b93a7)">${fmtF(p.fechaVence)}</span></span>
      <span>${proyTxt(p)}</span>
      <span>${realTxt(p)}</span>
      <span style="font-size:12px;color:var(--text-ter,#8b93a7)">${esc(p.fuente || 'manual')}</span>
    ...
    el.innerHTML = `
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin:12px 0">
        ${activos.length
          ...
          : `<span style="font-size:12px;color:var(--text-ter,#8b93a7)">Sin datos de cartera para elegir activo (reabrí el overlay con conexión)</span>`}
